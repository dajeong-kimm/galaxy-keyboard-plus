...
// Google Calendar 서버를 띄우는 IPC 핸들러
ipcMain.handle("gdrive-spawn", async () => {

  try {
    const gdriveServerDef = SERVER_DEFS.find((s) => s.id === "gdrive");
    ...
    if (gdriveServer) {
      log(`GDrive 서버 스폰 성공: ${gdriveServer.id}`);
      return { 
        success: true, 
        message: "Google Drive 인증이 완료되었습니다. 이제 GDrive 기능을 사용할 수 있습니다." 
      };
    } else {
      throw new Error("GDrive 서버 시작 실패");
    }
  } catch (e) {
    err("GDrive auth failed", e);
    return { 
      success: false, 
      message: `GDrive 인증 중 오류 발생: ${e.message}`,
      error: e.message 
    };
  }
